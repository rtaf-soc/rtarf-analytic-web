...
const Coveragedashboard: React.FC<KillChainDashboardProps> = ({
  ...
  const loadKillChainData = async () => {
    if (!selectedIndices.length) {
      setKillChainData(null); // reset data
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (!data || !data.phases || data.phases.length === 0) {
        ...
        console.warn("Empty data returned from API, using mock");
      ...
      setError("Failed to load cyber kill chain data, using mock");
      setKillChainData(killChainDataMock);
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative" style={{ minHeight: '700px' }}>
          ...
          <div className="relative flex justify-center gap-8"  style={{ minWidth: 'max-content', width: '100%' }}>
            {killChainData.phases.map((phase, index) => {
              ...
              const phasePalettes = [
                { big: '#cc2f2f', small: '#ff3b3b' }, // Red
                { big: '#cc632f', small: '#ff7b3b' }, // Orange
                { big: '#cc962f', small: '#ffbb3b' }, // Yellow-Orange
                { big: '#ccb22f', small: '#ffeb3b' }, // Yellow
                { big: '#7acc2f', small: '#9aff3b' }, // Light Green
                { big: '#2fcc7a', small: '#3bff9a' }, // Aqua Green
                { big: '#2f8ccc', small: '#3bbbff' }  // Blue
              ];
                  ...
                  <div className="relative z-10 mb-6 group">
                    ...
                    <div className="absolute -bottom-3 left-1/2 transform -translate-x-1/2">
                      <span
                        ...
                        style={{ backgroundColor: smallBaseColor }}
                      >
                        {index + 1}
                      </span>
                    </div>
      ...
      {/* Phase Details Panel */}
      {selectedPhaseData && (
        <div ref={detailsPanelRef} className="bg-gray-900 rounded-lg shadow-sm border border-gray-700 p-6">
